...
import { Filter, Loader2, Pencil, Trash2, UserPlus, X } from "lucide-react";

import { GenerateResetLinkDialog } from "@/modules/admin/components/generate-reset-link-dialog";
import { ResetPasswordDialog } from "@/modules/admin/components/reset-password-dialog";
import {
  DataTable,
  DataTablePagination,
  DataTableSearch,
  ...
  const columns: DataTableColumn<AdminUserRow>[] = [
    {
      ...
      cell: (user) => (
        <div className="flex items-center gap-2">
          ...
          {user.id === currentUserId ? <Badge variant="outline" className="border-slate-200 bg-white text-[10px] text-slate-500">You</Badge> : null}
        </div>
      ),
    },
    {
      ...
      <DataTable
        ...
        actions={
          <UserDialog
            currentUserId={currentUserId}
            onSaved={handleSaved}
            trigger={<Button size="sm" className="h-9 rounded-full"><UserPlus className="h-4 w-4" />New user</Button>}
          />
        }
